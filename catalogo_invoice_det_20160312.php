<?php 
	...
	if(isset($_REQUEST['idinvoice']))
	{
		...
		# Get Query Details
		$p_query_det="select cantidad,idcatalogo,pagina,sufijo,idproducto,color,talla,peso,subtotal,total 
...
<title>Catalogo de Invoices</title>
  ...
  <script type="text/javascript" src="librerias/calendarview/calendarview.js"></script>
    ...
    <style type="text/css">
		...
		.select-style{padding:2.5px; font-size:11px;}	
    </style>
	...
	function get_list_details()
	{
	  ...
	  { 
		...
		var item={cantidad:$('txt_cantidad'+nposition).value,
					idcatalogo:$('cb_catalogo'+nposition).value,
					idproducto:$('cb_producto'+nposition).value,
					idpagina:$('cb_pagina'+nposition).value,
					sufijo:$('txt_sufijo'+nposition).value,				
					color:$('txt_color'+nposition).value,
					tamanio:$('txt_longitud'+nposition).value,
					peso:$('txt_peso'+nposition).value,
					punitario:$('txt_punitario'+nposition).value,
					total:$('txt_total'+nposition).value} 
		list[nitem-1]=item; 
	  }
      return list;
	}	
	...
	function bt_agregar_click()
	{
		...
		var data="tipo_operacion=add-item&nitem="+nitem;

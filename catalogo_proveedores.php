...
/* --------------------------------------------------*/
	...
	$cmd="call sp_sis_get_proveedores();";
...
<head>
   ...
   <script type="text/javascript">
		...
		{ if(confirm("Desea agregar un nuevo Proveedor?"))
		  {add_supplier();}
		}
    ...
    <style type="text/css">
		.big{  height:100%; width:100%; position:absolute; top:0px
					; left:0px; display:block; background-color:#999; opacity:0.4; 
		...
		#layer{ position: absolute; top:20%; left:0; right:0; height:auto; width:auto; /*padding:10px 10px 10px 10px;  */
	     	...
	     	#layer div#div_layer{ width:auto; height:auto;padding:20px 0px 10px 10px;
					  border:1px solid #c6d5e1; background-color:#F8F8F9 }
		#layer div#div_layer table#tbdatos{ width:900px; }
		#layer div#div_layer table#tbdatos tr td#label{ width:14%; }
		#layer div#div_layer table#tbdatos tr td#value{ width:86%}
		#layer div#div_layer table#tbdatos tr td { padding:3px 0px 3px 0px; font-size:13px;}
		...
		#layer div#div_layer table#tbdatos tr td input[name="txt_nombre"],
		#layer div#div_layer table#tbdatos tr td input[name="txt_direccion"]{ width:350px;}
		...
		.item_over{ background-color:#538ECB;
				 width:600px; color:#FFF; height:20px;  cursor:pointer;
...
<body >
       <div id="big" class="big" style="display:none;">
          ...
          <div id="div_header" >
          		<label>Catalogo Proveedores </label>
          </div>
	  ...
	  <div id="layer" style="display:none; ">
		<form id="frm_edit" name="frm_edit">
			<input type="hidden" id="txt_idproveedor" name="txt_idproveedor" value="">
       		<div id="div_layer" >
				<table id="tbdatos">
					<tr>
						<td id="label"></td>
					...
					<tr>
						<td>Fch. Ingreso</td>
						<td ><input type="text" name="txt_fch_ingreso"  readonly="true" id="txt_fch_ingreso"></td>
					</tr>
					<tr>
						<td>Nombre</td>
					...
					<tr>
						<td>Direccion</td>
						<td ><input type="text" name="txt_direccion"  id="txt_direccion"></td>
					</tr>
					<tr>
						<td>Ciudad</td>
					...
					<tr>
						<td colspan="2">
							<input type="button" id="bt_guardar" name="bt_guardar" value="Guardar">
 ...
 <script type="text/javascript" >
			...
			if(confirm('Desea proceder a guardar los datos?'))
			{
				...
				new Ajax.Request("procesos.php",{method:'post',parameters:data,
										onSuccess: function(resultado){var response = resultado.responseText;
														...
														parent.middleside.location='catalogo_proveedores.php';}
														else
														{ alert(obj.descripcion); } 
														}});	  
			}
	...
	function edit_supplier(nfila)
	{
		
		var fondo= document.getElementById('big');
		fondo.className="big";
		Effect.toggle('big','appear');
		var operacion='get_supplier_values';
		var data="tipo_operacion="+operacion+"&idproveedor="+ nfila;
		...
		new Ajax.Request("procesos.php",{method:'post',parameters:data,
								onSuccess: function(resultado){var response = resultado.responseText;											
													...
													{
													  $('txt_idproveedor').value=result[0].codigo;

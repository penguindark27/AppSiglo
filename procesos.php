...
# it's to use json_encode and json_decode
	...
	switch($tipo_operacion)
	{
		case 'process-new-id':
				...
				$p_estado='0';
				$p_descripcion='';
				$cmd_validate="select idinvoice from sis_invoice where idinvoice='$p_id' ;";
				$result=mysqli_query($obj->cn,$cmd_validate);
				if(mysqli_num_rows($result)>0){
					 $p_estado='0'; $p_descripcion='Invoice Nro.'.$p_id.' Ya existe, intente generar uno nuevo.';
				}else{
					$cmd="insert into sis_invoice(idinvoice,fch_ingreso,nom_user) values('$p_id',now(),'$user') ;";
					if(mysqli_query($obj->cn,$cmd)){
						 $p_estado='1'; $p_descripcion='Invoice Generado Satisfactoriamente';
					} else{
						 $p_estado='2';  $p_descripcion=str_replace("'","\'",mysqli_error($obj->cn)); 
					}	
				}
				
				$lst_estado=array('idestado'=>$p_estado,'descripcion'=>$p_descripcion);
				echo json_encode($lst_estado);
			break;
		case 'get-new-id':
				$cmd="select lpad(convert(ifnull(max(idinvoice),0),unsigned)+1,6,'0') as  idinvoice from sis_invoice ; ";
		...
		case 'add-item-obs':			
			...
			?> 
				...
				<input type="hidden" id="row_type<?=$nitem;?>" value="edit" />
				<td colspan=10 style='text-align:center;'>
						<input type='text' id='cb_producto<?=$nitem;?>' style='width:430px' <?="class='$style'" ?>  />
				</td>
				<td style='text-align:center;'>
					<img src="imagenes/ico_remove.png" title="Eliminar" onclick='remove_item(<?=$nitem;?>)' style="border:0px; cursor:pointer;">
				</td>
			<?
			break;
		case 'add-item':			
			...
			?> 
				<input type="hidden" id="nposition" value="<?=$nitem;?>" />
				<input type="hidden" id="row_type<?=$nitem;?>" value="automatic" />
				<td>
